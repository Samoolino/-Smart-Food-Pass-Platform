...
contract SettlementAnchor is AccessControl, ISettlementAnchor {
    ...
    {
        if (batch.batchId == bytes32(0)) revert Errors.InvalidIdentifier();
        if (batch.merchantId == bytes32(0)) revert Errors.InvalidIdentifier();
    ...
    function markBatchFailed(bytes32 batchId) external onlyRole(Roles.SETTLEMENT_SIGNER_ROLE) {
        ...
        if (batch.status == DataTypes.SettlementStatus.FAILED) revert Errors.InvalidPolicy();
